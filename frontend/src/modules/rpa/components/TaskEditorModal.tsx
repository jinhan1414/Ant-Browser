...
import { TaskProfileSelector } from './TaskProfileSelector'
...
interface TaskFormSectionProps {
  ...
  task: RPATask
  setTask: Dispatch<SetStateAction<RPATask>>
}

function TaskFormSection({ flowOptions, task, setTask }: TaskFormSectionProps) {
  return (
...
interface ScheduledTaskSectionProps {
  scheduleConfig: RPATask['scheduleConfig']
  setTask: Dispatch<SetStateAction<RPATask>>
}

function ScheduledTaskSection({ scheduleConfig, setTask }: ScheduledTaskSectionProps) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 rounded-lg border border-[var(--color-border-default)] p-4 bg-[var(--color-bg-secondary)]/40">
      <FormItem label="Cron 表达式" required hint="示例：0 9 * * * 表示每天 09:00">
        <Input
          value={normalizeScheduleConfig(scheduleConfig).cron}
          onChange={e => setTask(prev => ({
            ...prev,
            scheduleConfig: {
      ...
      <FormItem label="时区">
        <Select
          value={normalizeScheduleConfig(scheduleConfig).timezone}
  ...
  return (
    ...
    >
      <div className="space-y-4">
        <TaskFormSection flowOptions={flowOptions} task={task} setTask={setTask} />
        {task.taskType === 'scheduled' && <ScheduledTaskSection scheduleConfig={task.scheduleConfig} setTask={setTask} />}
        <FormItem label="启用状态">
        ...
        <FormItem label="执行环境">
          <TaskProfileSelector
            ...
            resetKey={`${open}-${initialTask?.taskId || 'new'}-${initialTargets.map(item => item.profileId).join(',')}`}
